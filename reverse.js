var SVGPathEditor = (function() {
  return {
     ...
     */
    reverseNormalizedPath: function (normalized) {
      var terms = normalized.split(' '), tlen = terms.length, tlen1 = tlen-1, t, reversed = [], x, y, pair, pairs, shift;
     ...
     */
    reverseSubPath: function (path, subpath) {
      var path = this.normalizePath(path),
          paths = path.replace(/M/g,'|M').split("|"),
          revpath;
      paths.splice(0,1);
      if (subpath >= paths.length) {
        return path;
      }
      var spath = paths[subpath]
      if (spath) {
        revpath = this.reverseNormalizedPath(spath.trim());
